...
#Option to install fail2ban
enable_f2ban=1
read -p "Do you wish to install with fayl2ban? [Y/N, Default:Yes] " yn
case $yn in
    [Nn]* ) enable_f2ban=0;;
    * ) enable_f2ban=1;;
esac
...
#Local server IP
ip4=$(/sbin/ip -o -4 addr list enp0s18 | awk '{print $4}' | cut -d/ -f1)
echo "Local IP:$ip4"
...
if [ $enable_lets -eq 0 ]; then
    #Set SelfSigned cert
sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/private/nginx-selfsigned.key -out /etc/ssl/certs/nginx-selfsigned.crt
sudo openssl dhparam -out /etc/nginx/dhparam.pem 4096

self-signed-conf="$(cat << EOF
ssl_certificate /etc/ssl/certs/nginx-selfsigned.crt;
ssl_certificate_key /etc/ssl/private/nginx-selfsigned.key;
EOF
)"
 echo "${self-signed-conf}" > /etc/nginx/snippets/self-signed.conf
